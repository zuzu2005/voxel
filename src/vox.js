...
function Chunk(cbuf,offset){
    var buf = new Int32Array(cbuf.buffer,offset);
    this.buf = buf;
    this.name = int2tag(buf[0]);
    this.size = buf[1];
    this.childrenSize = buf[2];
    this.readSubOffset = offset;
    if(this.size>0)
        this.data = new Int32Array(buf.buffer,cbuf.byteOffset+12,this.size/4);
    if(this.childrenSize>0)
        this.childrenBuf = new Int32Array(buf.buffer,cbuf.byteOffset+12+this.size,this.childrenSize/4);
}
